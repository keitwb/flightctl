...
var _ = Describe("DeviceStore create", func() {
	...
	Context("Device store", func() {
		...
		It("UpdateRendered forceUpdate bypasses the specUnchanged short-circuit", func() {
			...
			// Establish an initial rendered version for a given specHash with a non-empty osImage.
			firstVersion, err := devStore.UpdateRendered(ctx, orgId, "dev-force-update", config, "", "samehash", "quay.io/org/os:v1", nil, false)
			Expect(err).ToNot(HaveOccurred())
			...
			noopVersion, err := devStore.UpdateRendered(ctx, orgId, "dev-force-update", config, "", "samehash", "quay.io/org/os:v1", nil, false)
			Expect(err).ToNot(HaveOccurred())
			...
			forcedVersion, err := devStore.UpdateRendered(ctx, orgId, "dev-force-update", config, "", "samehash", "quay.io/org/os:v1", nil, true)
			Expect(err).ToNot(HaveOccurred())
			...
			clearedVersion, err := devStore.UpdateRendered(ctx, orgId, "dev-force-update", config, "", "clearhash", "", nil, false)
			Expect(err).ToNot(HaveOccurred())
