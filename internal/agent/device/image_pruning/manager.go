...
type ImageArtifactReferences struct {
	...
	// Deprecated in favor of ReferencesWithOwner. Old data will still be read from this field but
	// only written to ReferencesWithOwner.
	References []string `json:"references"` // Single list of all references (no categorization)
	// Replacement for References that includes user ownership information.
	ReferencesWithOwner []ImageRef `json:"referencesWithOwner"`
}
...
// GetReferences allows the
func (i ImageArtifactReferences) GetReferences() []ImageRef {
	out := slices.Clone(i.ReferencesWithOwner)
	for _, r := range i.References {
		out = append(out, ImageRef{Image: r})
	}
	return out
}
...
func (m *manager) recordImageArtifactReferences(ctx context.Context, current, desired *v1beta1.Device) error {
	...
	existingReferences := existingRefs.GetReferences()
	...
	refs := ImageArtifactReferences{
		...
		ReferencesWithOwner: lo.Uniq(existingReferences),
	}
	...
	m.log.Debugf("Recorded image/artifact references to %s (accumulated: %d references)", filePath, len(refs.GetReferences()))
	return nil
}
...
func (m *manager) removePrunedReferencesFromFile(removedImages []ImageRef, removedArtifacts []ImageRef) error {
	...
	filteredReferences := lo.Without(lo.Uniq(existingRefs.GetReferences()), lo.Uniq(allRemoved)...)

	// Build updated references
	refs := ImageArtifactReferences{
		Timestamp:           time.Now().UTC().Format(time.RFC3339),
		ReferencesWithOwner: filteredReferences,
	}
...
func (m *manager) determineEligibleImages(ctx context.Context) (*EligibleItems, error) {
	...
	previousReferences := previousRefs.GetReferences()
