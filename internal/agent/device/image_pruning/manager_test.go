	...
	testCases := []struct {
	...
	}{
		{
			...
			setupMocks: func(mockExec *executer.MockExecuter, mockSpec *spec.MockManager, readWriter fileio.ReadWriter, dataDir string) {
				...
				previousRefs := ImageArtifactReferences{
					...
					ReferencesWithOwner: []ImageRef{{Image: "quay.io/example/old-app:v1.0"}, {Image: "quay.io/example/unused:v1.0"}, {Image: "quay.io/example/artifact:v1.0"}},
				}
		...
		},
		{
			...
			setupMocks: func(mockExec *executer.MockExecuter, mockSpec *spec.MockManager, readWriter fileio.ReadWriter, dataDir string) {
				...
				previousRefs := ImageArtifactReferences{
					Timestamp:           "2025-01-01T00:00:00Z",
					ReferencesWithOwner: []ImageRef{{Image: "quay.io/example/app:v1.0"}},
				}
				jsonData, err := json.Marshal(previousRefs)
				require.NoError(err)
				// Ensure directory exists
		...
		},
		{
			...
			setupMocks: func(mockExec *executer.MockExecuter, mockSpec *spec.MockManager, readWriter fileio.ReadWriter, dataDir string) {
				...
				previousRefs := ImageArtifactReferences{
					Timestamp:           "2025-01-01T00:00:00Z",
					ReferencesWithOwner: []ImageRef{{Image: "quay.io/example/unused:v1.0"}, {Image: "quay.io/example/old-os:v1.0"}},
				}
				jsonData, err := json.Marshal(previousRefs)
				require.NoError(err)
				// Ensure directory exists
