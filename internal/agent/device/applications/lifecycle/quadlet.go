...
func NewQuadlet(log *log.PrefixLogger, rw fileio.ReadWriter, systemdFactory systemd.ManagerFactory, podmanFactory client.PodmanFactory) *Quadlet {
	return &Quadlet{
...
func (q *Quadlet) Execute(ctx context.Context, actions ...*Action) error {
	...
	for _, action := range actions {
		...
		case ActionUpdate:
			...
			if err := q.add(ctx, action); err != nil {
				return fmt.Errorf("adding for update: %w", err)
			}
		default:
